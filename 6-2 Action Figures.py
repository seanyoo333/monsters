...
def box_ok(box):
    ...
    True를 반환하면 상자 내 액션 피규어들으 ㅣ높이의 순서가 작아지지 않는 것이고,
    False는 그렇지 않은 것이다"""
...
def all_boxes_ok(boxes):
    """ boxes는 삼자들의 리스트, 각 상자는 액션피규어 높이들의 리스트
...
def boxes_endpoints(boxes):
    """각각 두개의 값이 있는 리스트들을 값으로 갖는 리스트를 반화 두개의 값은 상자의 가장 왼쪽과 가장 오른쪽 액션 피규어의 높이"""
    endpoints = []
    for box in boxes:
